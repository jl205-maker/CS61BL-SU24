...
public class Model {
    ...
    public boolean emptySpaceExists() {
        //traverse the board to check for null
    ...
    public boolean maxTileExists() {
        //traverse the board to check for winning value
        for (int row = 0; row < this.size(); row++) {
            for (int col = 0; col < this.size(); col++) {
                Tile currentTile = this.tile(row, col);
        ...
        if (this.emptySpaceExists()) {
            ...
            for (int row = 0; row < this.size(); row++) {
                for (int col = 0; col < this.size(); col++) {
                    Tile currentTile = this.tile(row, col);
                    //check right
                    if (col + 1 < this.size() && this.tile(row, col + 1).value() == currentTile.value()) {
                        return true;
                    }

                    //check left
                    if (col - 1 >= 0 && this.tile(row, col - 1).value() == currentTile.value()) {
                        return true;
                    }

                    //check down
                    if (row + 1 < this.size() && this.tile(row + 1, col).value() == currentTile.value()) {
                        return true;
                    }
                    
                    //check up
                    if (row - 1 >= 0 && this.tile(row - 1, col).value() == currentTile.value()) {
                        return true;
                    }
                }
            }
    ...
    public void moveTileUpAsFarAsPossible(int x, int y) {
        ...
        int targetY = y;
        //task5: move tile up
        //task6: when a tile move up and it sees an empty square, perform the move; when it sees a tile with
        //the same value as its own and this upper tile has not been merged yet, do the merge.
        boolean aboveAvailable = true;
        while ((aboveAvailable) && (targetY < this.size() - 1)) {
    ...
    public void tiltColumn(int x) {
        //tile one column up
        for (int currY = this.size() - 1; currY >= 0; currY--) {
            if (board.tile(x, currY) != null) {
                moveTileUpAsFarAsPossible(x, currY);
            }
        }
    }

    public void tilt(Side side) {
        //tiles a board to the input side
