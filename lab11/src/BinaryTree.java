...
public class BinaryTree<T> {
    ...
    public int height() {
        //call the height helper method
        return heightHelper(root);
    }

    private int heightHelper(TreeNode node) {
        // recursively computing the height of a binary tree
        if (node == null) {
            return 0;
        }
    ...
    public static BinaryTree<Integer> fibTree(int N) {
        BinaryTree<Integer> result = new BinaryTree(fibNode(N));
        return result;
    }

    private static TreeNode<Integer> fibNode(int N) {
        TreeNode<Integer> root = new TreeNode<>(N);
        if (N <= 1) {
            return root;
        }
        TreeNode<Integer> left = fibNode(N - 1);
        TreeNode<Integer> right = fibNode(N - 2);
