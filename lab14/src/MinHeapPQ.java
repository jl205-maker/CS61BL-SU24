...
public class MinHeapPQ<T> implements PriorityQueue<T> {
    ...
    public void insert(T item, double priorityValue) {
        PriorityItem itemToInsert = new PriorityItem(item, priorityValue);
        if (heap.contains(itemToInsert)) {
            throw new IllegalArgumentException("Item already exists in the queue.");
        }
    ...
    public void changePriority(T item, double priorityValue) {
        PriorityItem itemToModify = new PriorityItem(item, priorityValue);
        if (!heap.contains(itemToModify)) {
            throw new NoSuchElementException("Priority item does not exist in the queue.");
        }
